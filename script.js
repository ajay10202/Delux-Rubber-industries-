...
window.addEventListener("beforeprint",()=>{
  // Keep at least one row, but remove completely unused extra rows from print.
  [...body.rows].forEach(r=>{
    ...
    if(!desc && !hsn && Number(qty)===0 && Number(rate)===0 && body.rows.length>1) r.remove();
  });
  renumber();
  calc();
});
